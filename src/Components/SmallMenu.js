    ...
    return (
			<div>
				<div className="one-button-header z-10">
					...
					<p className="text-lg text font-semibold m-auto text-center w-full">
						{headerText}
					</p>
				</div>
						...
						item && (
							<animated.div style={styles}>
								<div
									...
									onClick={() => setMenuIsActive(false)}></div>
							</animated.div>
						)
				)}
				...
				{menuTransitions(
					(styles, item) =>
						item && (
							<animated.div style={styles}>
								<div className="bg-white dark:bg-gray-900 fixed left-0 w-4/5 top-0 bottom-0 pb-17 px-6 pt-4 overflow-scroll overscroll-contain ">
									<Menu />
								</div>
							</animated.div>
						)
